...
fun ConfirmacaoExclusaoDialog(
    ...
    AlertDialog(
        ...
        text = {
            Text(text = "Tem certeza de que deseja excluir a tarefa \"${tarefa.titulo}\"? Esta ação não poderá ser desfeita.")
        },
        confirmButton = {
...
@Composable
private fun ListaTarefasContentComDialogoPreview() {
    val sampleTarefa = Tarefa(id = 1, titulo = "Estudar Room", descricao = "Revisar anotações e DAO", concluida = false)
    ListaTarefasContent(
        tarefas = listOf(
            sampleTarefa,
            Tarefa(id = 2, titulo = "Enviar atividade", descricao = "Upload no portal da FIAP", concluida = true)
        ...
        onCheckedChange = { _, _ -> },
        onDeletar = {},
        tarefaParaExcluirInicial = sampleTarefa
    )
}
